  ...
  for (i = 0; i < local_size; i++) { //rows in multiply
    for (j = 0; j < local_size; j++) { //columns in multiply
      for (k = 0; k < local_size; ) { //columns in first and rows in second
        if (k < local_size - 4) {
          register double ikj = first[i][k]*second[k][j];
          register double ik1j = first[i][k+1]*second[k+1][j];
          register double ik2j = first[i][k+2]*second[k+2][j];
          register double ik3j = first[i][k+3]*second[k+3][j];
