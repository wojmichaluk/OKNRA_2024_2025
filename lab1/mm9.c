  ...
  for (i = 0; i < local_size; i++) { //rows in multiply
    for (j = 0; j < local_size; j++) { //columns in multiply
      for (k = 0; k < local_size; ) { //columns in first and rows in second
        if (k < local_size - 4) {
          register double ikj = _first_(i, k)*_second_(j, k);
          register double ik1j = _first_(i, k+1)*_second_(j, k+1);
          register double ik2j = _first_(i, k+2)*_second_(j, k+2);
          register double ik3j = _first_(i, k+3)*_second_(j, k+3);
