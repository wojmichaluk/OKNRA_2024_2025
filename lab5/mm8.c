...
int mm(double ** first, double ** second, double ** multiply, int SIZE)
{
  ...
  double local_multiply[BLOCK_SIZE * BLOCK_SIZE];
  ...
  for (ii = 0; ii < SIZE; ii += BLOCK_SIZE) {
    for (jj = 0; jj < SIZE; jj += BLOCK_SIZE) {
      ...
      for (kk = 0; kk < SIZE; kk += BLOCK_SIZE) {
        for (i = 0; ii + i < SIZE && i < BLOCK_SIZE; i++) {
          for (j = 0; kk + j < SIZE && j < BLOCK_SIZE; j++) {
            local_first[i * BLOCK_SIZE + j] = first[ii+i][kk+j];
          }
        }
        for (i = 0; i < BLOCK_SIZE && (kk + i) < SIZE; i++) {
          for (j = 0; j < BLOCK_SIZE && (jj + j) < SIZE; j++) {
            local_second[i * BLOCK_SIZE + j] = second[kk+i][jj+j];
          }
        }
        for (i = 0; i < BLOCK_SIZE && (ii + i) < SIZE; i++) { //rows in multiply
          for (k = 0; k < BLOCK_SIZE && (kk + k) < SIZE; k++) { //columns in first and rows in second
            a_val = local_first[i * BLOCK_SIZE + k];
            for (j = 0; j < BLOCK_SIZE && (jj + j) < SIZE; j++) { //columns in multiply
              local_multiply[i * BLOCK_SIZE + j] += a_val * local_second[k * BLOCK_SIZE + j];
            }
          }
        }
      }
